...
function showBox(theTarget) {
	...
	theLoading.style.top = parseInt((pageCoords[0] - 55) / 2) + 'px';
	theLoading.onclick = function() { closeBox(); }
	theShadow.appendChild(theLoading);

	var imgPreload = new Image();
	imgPreload.onload = function() {
		let widthM = imgPreload.width > 720 ? 720:imgPreload.width;
		let heightM = imgPreload.height > 405 ? 405:imgPreload.height;
		var theBox = document.createElement('div');
		theBox.id = 'litebox';
		theBox.style.width = widthM + 10 + 'px';
		theBox.style.marginTop = parseInt((pageCoords[0] - heightM - 50) / 2) + 'px';

		var theImage = document.createElement('img');
		theImage.src = theTarget.href;
		theImage.alt = theTarget.title;
		theImage.width = widthM;
		theImage.onclick = function() { closeBox(); }
		theImage.title = "";

		var theCaption = document.createElement('p');
		theCaption.innerHTML = (theImage.alt) ? theImage.alt : '';
		theCaption.innerHTML += "";
